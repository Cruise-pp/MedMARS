...
def knowledge_diagnosis(state: AgentState) -> dict:
    """
    TODO:
      1. Combine user_text + clinical_evidence (if any) into SFT prompt
         (same format as ddxplus_sft.py output)
      2. Call fine-tuned Mistral-7B-Instruct LoRA
      3. Parse JSON → primary_diagnosis + differential_diagnosis
    """
    user_text = state.get("user_text", "")
    evidence  = state.get("clinical_evidence", "")
    print(f"[Knowledge-Diagnosis] query='{user_text[:60]}…'")

    diagnosis_output = json.dumps({
        "primary_diagnosis": "[STUB]",
...
# ================================================================
# 5c. Knowledge Agent — General QA  (STUB — MedQuAD RAG + LLM)
# ================================================================
def knowledge_general(state: AgentState) -> dict:
    """
    TODO:
      1. Embed user_text → query MedQuAD FAISS index → top-k snippets
      2. Feed (query + snippets) to LLM
    """
    user_text = state.get("user_text", "")
    print(f"[Knowledge-General] query='{user_text[:60]}…'")

    diagnosis_output = json.dumps({
        "answer": "[STUB] General medical QA answer",
    })
    return {"diagnosis_output": diagnosis_output}
